/**
 * Seed script — injecte les 44 articles par défaut dans toutes les orgs.
 * Usage: npx ts-node --compiler-options '{"module":"commonjs","moduleResolution":"node"}' seed-articles.ts
 * Ou: utilise le bouton "Seeder" sur /app/parametres/defauts après connexion.
 */
import { createClient } from "@supabase/supabase-js"

const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || "https://bxveolgqxtnybscfriyy.supabase.co"
const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY || ""

if (!serviceKey) {
  console.error("SUPABASE_SERVICE_ROLE_KEY required — run from Vercel env or .env.local")
  process.exit(1)
}
...
async function main() {
  const { data: orgs } = await admin.from("orgs").select("id, nom")
  ...
  for (const org of orgs) {
    ...
    const { data: existing } = await admin
      ...
      .select("nom")
      .eq("org_id", org.id)
    const existingNames = new Set((existing ?? []).map((e: any) => e.nom.toLowerCase()))
    const newOnes = DEFAULT_ARTICLES.filter((a) => !existingNames.has(a.nom.toLowerCase()))
      .map((a) => ({ ...a, org_id: org.id }))

    if (newOnes.length === 0) {
      console.log(`[${org.nom}] Aucun nouvel article (${existing?.length ?? 0} déjà présents)`)
      continue
    }

    const { error } = await admin.from("articles").insert(newOnes)
    if (error) {
      console.error(`[${org.nom}] Erreur:`, error.message)
    } else {
      console.log(`[${org.nom}] ${newOnes.length} articles ajoutés (${existing?.length ?? 0} existants)`)
    }
  }
}

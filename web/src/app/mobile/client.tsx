...
export function MobileHome({
  ...
  return (
    <div className="min-h-screen bg-[#07070b] flex flex-col">
      {/* Header */}
      <div className="bg-[#0e0e17] border-b border-[#1e1e2e] px-4 py-4 flex items-center justify-between">
        <div>
          <div className="text-xl font-bold text-white">⚡ DEP</div>
          <div className="text-xs text-[#f5c518]">Bonjour, {userName.split(" ")[0] || "vous"} 👋</div>
        </div>
        <button onClick={logout} className="p-2 rounded-xl text-gray-500 hover:text-white">
      ...
      {/* Installer l'app */}
      {!isInstalled && <div className="mx-4 mb-4 bg-[#0e0e17] border border-[#f5c518]/30 rounded-2xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          <div className="text-2xl">📲</div>
          <div className="text-sm font-bold text-white">Installer DEP sur votre téléphone</div>
        </div>
        <div className="space-y-2">
          <div className="text-xs font-semibold text-[#f5c518]">Sur iPhone (Safari) :</div>
          <div className="flex items-start gap-2 text-xs text-gray-300">
            <span className="bg-[#f5c518] text-black rounded-full w-4 h-4 flex items-center justify-center font-bold shrink-0">1</span>
            <span>Appuyez sur l&apos;icône <strong>□↑</strong> en bas de Safari</span>
          </div>
          <div className="flex items-start gap-2 text-xs text-gray-300">
            <span className="bg-[#f5c518] text-black rounded-full w-4 h-4 flex items-center justify-center font-bold shrink-0">2</span>
            <span>Faites défiler et appuyez sur <strong>"Sur l'écran d'accueil"</strong></span>
          </div>
          <div className="flex items-start gap-2 text-xs text-gray-300">
            <span className="bg-[#f5c518] text-black rounded-full w-4 h-4 flex items-center justify-center font-bold shrink-0">3</span>
            <span>Appuyez sur <strong>"Ajouter"</strong> — c'est fait ! ✅</span>
          </div>
        </div>
        <div className="space-y-2">
          <div className="text-xs font-semibold text-[#f5c518]">Sur Android (Chrome) :</div>
          <div className="flex items-start gap-2 text-xs text-gray-300">
            <span className="bg-[#f5c518] text-black rounded-full w-4 h-4 flex items-center justify-center font-bold shrink-0">1</span>
